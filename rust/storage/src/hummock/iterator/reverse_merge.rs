...
use crate::hummock::{iterator::merge_inner::MergeIteratorInner, memtable::SkiplistMemTable};

pub type ReverseMergeIterator<'a> = MergeIteratorInner<'a, BACKWARD, SkiplistMemTable>;
...
mod test {
    ...
    async fn test_reverse_merge_invalidate_reset() {
        ...
        let iters: Vec<IteratorType> = vec![
            IteratorType::new_sstable_iterator(Box::new(ReverseSSTableIterator::new(
                Arc::new(table1),
            ))),
            IteratorType::new_sstable_iterator(Box::new(ReverseSSTableIterator::new(
                Arc::new(table0),
