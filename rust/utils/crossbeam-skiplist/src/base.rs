...
impl<K, V> Node<K, V> {
    /// Allocates a node.
    ///
    /// The returned node will start with reference count of `ref_count` and the tower will be
    /// initialized with null pointers. However, the key and the value will be left
    /// uninitialized, and that is why this function is unsafe.
    ...
    K: Ord,
{
        ...
        F: FnOnce() -> V,
    {
        ...
        unsafe {
            ...
            // create value before creating node, so extra allocation doesn't happen if value()
            // function panics
            let value = value();
