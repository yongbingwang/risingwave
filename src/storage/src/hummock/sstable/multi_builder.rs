...
pub struct CapacitySplitTableBuilder<B, W: SstableWriter<WO>, WO> {
    /// When creating a new [`SSTableBuilder`], caller use this closure to specify the id and
    /// options.
    get_id_and_builder: B,

    finished_ssts: Vec<(u64, SSTableBuildOutput<WO>)>,

    current_builder: Option<(u64, SSTableBuilder<W, WO>)>,
}

impl<B, F, W, WO> CapacitySplitTableBuilder<B, W, WO>
where
    B: FnMut() -> F,
    ...
    W: SstableWriter<WO>,
{
    ...
    /// will be no-op.
    pub async fn seal_current(&mut self) -> Result<()> {
        if self.current_builder.is_some() {
    ...
    pub async fn finish(mut self) -> Result<Vec<(u64, SSTableBuildOutput<WO>)>> {
